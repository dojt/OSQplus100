...
* MORE General Blah
      ...
      * When figuring out processes that modify a (bunch of) logical qubit(s) using initializations and
        ...
        freedom are right (i.e., no bad stabilizers — there'll never be too few stabilizers if the
        logical-contribution arithmetic (see [["Logical contributions" of physical qubit neighborhoods]] below) is
        right).

      * 🤔🤔🤔 :: Why do [fn:river] (Fig. 10) end the rotation with a /shrink/ and then move(swap) the patch one
...
** ↘️ Code
*** MORE Shapes
***** Textbook shape 𝑑=7: 7×7 physical qubits
        + Stab-picture:
          #+BEGIN_EXAMPLE
           ∧ ∧ ∧
           XZXZXZ⟩
...
***** Textbook expansion (growing) leads to 𝑑×𝑒 blocks, with both 𝑑,𝑒 odd and ≥3

***** The condition 𝑑,𝑒 odd can be dropped.  Then errors of weight 𝑑/2 or 𝑒/2 can only be detected, not
        corrected.
...
***** Deformed rectangular :: See [fn:FoGi] Fig. 13, or [fn:river] Fig. 10(b).
        ...
        + In terms of Z/X-boundaries, the edge in the example is split between the part left of (or above) the skip
          — in the example, that's a Z-boundary — and the part right of (or below) the split — in the example,
...
***** Convex corner 𝛔-𝛔 near :: 1-(1/4+1/2+1/2)                                      = -1/4
        #+NAME: Convex corner 𝛔-𝛔 near
        #+BEGIN_EXAMPLE
        ∧   ∧
         ⋅ ⋅ ⊙
        Z X Z ⟩
...
***** Convex corner obtuse 𝛔-𝛔 near :: 3 − 2⋅(1/2+1/4+1/3) − (1/4+1/4+1/4+1/3)       = -1/4
        #+NAME: Convex corner obtuse 𝛔-𝛔 near
        #+BEGIN_EXAMPLE
          ∧
         ⋅ ⊙
        Z X ◺
...
***** Convex corner 𝛔-𝛔 far :: 1-1/4                                                 = +3/4 = /B.S./
        #+BEGIN_EXAMPLE
          ∧
         ⋅ ⋅ ⊙
        Z X Z
...
***** Convex corner obtuse 𝛔-𝛔 far :: 3 − 2⋅(1/4+1/3) − (1/4+1/4+1/4+1/3)            = +3/4 = /B.S./
        #+BEGIN_EXAMPLE
        ∧
         ⋅ ⊙
        Z X ◺
...
***** Concave corner 𝛔-𝛔 near :: 1-(1/4+1/4+1/4)                                     = +1/4
        #+BEGIN_EXAMPLE
        ⋅ ⋅ ⋅ ⋅
         X Z X ⟩
        ⋅ ⋅ ⋅ ⋅
...
***** Concave corner 𝛔-𝛔 far+ramp :: 3 − 2⋅(1/4+1/4+1/3) − (1/4+1/4+1/4+1/3)         = +1/4
        #+BEGIN_EXAMPLE
        ⋅ ⋅ ⋅ ⋅
         Z X Z ⟩
        ⋅ ⋅ ⋅ ⋅
...
***** ❓❓❓ :: What is Fig. 9(b) in [fn:FoGi] ???

***** etc

**** Notes

      - »Concave corner 𝛔-𝛔 near« and »Concave corner 𝛔-𝛔 far+ramp« can be combined to give a 180° turn, as in
...
** ↘️ Shape changes & moves; Merge & Split
*** ↘️ Expand & Shrink
***** MORE Expand

      Aka "grow"

      * Expand along 𝞂-boundary

        + Process:
          1. Prepare all new qubits in +𝞂 state
        ...
        + /Is this correct?!?/ — What you have to check:
          a) Check degrees of freedom: \\
             ...
             freedom left.
             - Idea :: Maybe that's a cool way for log-qubit reset? 🤔
          b) Logical state before ⟿ after \\
...
***** MORE FoGi rotate
      ...
      * There are loads of choices, e.g.,:
        - [fn:FoGi] puts the [[Convex corner obtuse 𝛔-𝛔 near][obtuse corner]] at the end of the edge which contains the [[Skip 1 on an edge][skip-1]], while
        - [fn:river] put the [[Convex corner obtuse 𝛔-𝛔 near][obtuse corner]] at the end of the edge opposite of the edge with the [[Skip 1 on an edge][skip-1]].
        - The protocol is different depending on even/odd-ness of rows/columns: the -1/4 (on a corner) that's
          necessary to cancel the +1/2 from the [[Skip 1 on an edge][skip-1]] can be a »[[Convex corner obtuse 𝛔-𝛔 near]]« (odd[fn:FoGi]) or a
          »[[Convex corner 𝛔-𝛔 near]]« (even[fn:river]).
      ...
      * TODO :: 🤔🤔 On the one hand, pseudocode for this will be a PITA because there are so many cases.  On the
...
***** S-gate
      ...
      * Magic state :: <<¤S-magic>>
        + Prepare 𝗦|+⟩ state on aux, measure Z⊗Z, measure Xₐᵤₓ, fix[fn:river]
          - ZX-diagram:
            #+BEGIN_EXAMPLE
             +----------------+
             |                |
            ...
            ─────────Z───Zₜ₊ₐ───
             ...
             +----------------+
            #+END_EXAMPLE
        + Simpler: 𝗦|+⟩ state on aux, merge, fix:
          - ZX-diagram (also fused ~Z─Z½─~ to ~Z½─~):
            #+BEGIN_EXAMPLE
             +------------+
             |            |
             | Z½         |
...
***** ARGH T-gate
      ...
      * Gate teleportation when magic state |0⟩+√𝒊|1⟩ is available
        + Similar to [[¤S-magic]]
        + ARGH :: ZX-diagram (also fused ~Z─Z½─~ to ~Z½─~):
          #+BEGIN_EXAMPLE
           +------------+
           |            |
           | Z¼         |
...
** Online discussions
  ...
  * [[https://quantumcomputing.stackexchange.com/questions/38232/deformation-rules-for-zx-pipe-diagrams][Gidney's reply to /Deformation rules for ZX-pipe diagrams/.]]  DOT: Has recommended reading! 😊
